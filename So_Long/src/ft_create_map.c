...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/04/14 13:44:27 by ecayir            #+#    #+#             */
/*   Updated: 2022/04/14 16:21:37 by ecayir           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_mlx_put_window(t_type **mcq, void *obje, int x, int y)
{
	mlx_put_image_to_window((*mcq)->mlx, (*mcq)->window,
		obje, x * (*mcq)->picture_size, y * (*mcq)->picture_size);
}
...
void	ft_create_objects(t_type **mcq, char obje, int x, int y)
{
	...
	{
		(*mcq)->diamond_count++;
		ft_mlx_put_window(mcq, (*mcq)->diamond, x, y);
	}
	else if (obje == 'P')
	{
		ft_mlx_put_window(mcq, (*mcq)->player, x, y);
		(*mcq)->player_x = x;
		(*mcq)->player_y = y;
	}
	else if (obje == 'E')
